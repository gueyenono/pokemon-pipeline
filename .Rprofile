...
#   at run-time.
{
    ...
    # R.nvim (Neovim) spawns its own R child processes (identifiable via
    # RNVIM_COMPLDIR) and needs a writable library to install/load its
    # `nvimcom` helper package, which gates `rnvimserver` startup. Stripping
    # R_LIBS_USER for those sessions breaks R.nvim, so exempt them from the
    # purity check below. Note: this is a local, hand-added exemption --
    # rerunning rix::rix_init()/rix(overwrite = TRUE) regenerates this file
    # from scratch and will remove it.
    is_rnvim <- nzchar(Sys.getenv("RNVIM_COMPLDIR"))
    ...
    rm(is_rstudio, is_nix_r, is_rnvim)
}
